...
Sign:
    ...
    0 is an error. If Sign=SIGN for a particular HSYNC record, then the
...
Provider is currently a designated DNS Provider for the zone (or in
the process of being onboarded) and "OFF" means that the DNS Provider
was previously a designated DNS Provider for the zone that is in the
...
data may be removed in the correct order, either during the multi-signer
...
this case these Agents MUST instruct the local Combiner to update the NS
...
"SIGN" means that the Agent represents a DNS Provider that should
